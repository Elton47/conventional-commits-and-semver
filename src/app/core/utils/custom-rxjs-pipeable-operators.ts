...
function createOnDestroySubjectAndReturnIt<T>(classInstance: T & autoUnsubscribeClassExtendedTypeChecking): Subject<void> {
 ...
 *
 * _Should be used in pair with the custom_ {@link AutoUnsubscribe AutoUnsubscribe} _class decorator (see example below)_
 *
 * @param classInstance
 ...
 * }
 */
export function unsubscribeOnDestroy<T>(classInstance: T & autoUnsubscribeClassExtendedTypeChecking) {
  return <U>(source: Observable<U>) => {
    if (!classInstance.autoUnsubscribeDecoratorAttached) {
      throw new Error(`The class (${classInstance.constructor.name}), where the custom "unsubscrieOnDestroy" pipeable operator is being used, is not decorated with "AutoUnsubscribe" decorator`);
    }
    return source.pipe(takeUntil<U>(createOnDestroySubjectAndReturnIt(classInstance.constructor.prototype)));
  }
}
